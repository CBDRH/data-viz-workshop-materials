...
download.file(url, 'raw-data/national-health-survey-2022-table-1.xlsx', mode = "wb")

# Read in the raw data
df2Raw <- openxlsx::read.xlsx(
  xlsxFile = 'raw-data/national-health-survey-2022-table-1.xlsx',
  sheet = 'Table 1.3_Proportions',
  startRow = 6
)

# Make sure column names are unique
names(df2Raw) <- make.unique(names(df2Raw))

# Clean the raw data
df2Clean <- df2Raw |>
  slice(12:23) |>
  select(c(X1, ends_with(".1"))) |>
  pivot_longer(
    cols = ends_with(".1"),
    names_to = 'period',
    values_to = 'percent',
  ...
  mutate(
    date = as.Date(
      ifelse(nchar(period)==4,
             paste(period, '07', '01', sep='-'),
             paste(as.numeric(substr(period, 1, 4)) + 1, '01', '01', sep='-'))
    ),
    condition = case_when(
  ...
  select(condition, date, percent)

# Save the data in .Rda format
save(df2Clean, file=here::here('clean-data/vis2.Rda'))

# Save the data in .csv format
write.csv(df2Clean,
          file=here::here('clean-data/vis2.csv'),
          row.names = FALSE)
